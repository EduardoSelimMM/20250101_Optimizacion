...
+ En el objeto `datos` ya están todos los datos.

+ Vamos a dividir en datos de entrenamiento y datos de prueba

```{r}
set.seed(28032005)
split_datos <- rsample::initial_split(datos, prop = .7) # Usaré el 70% de los datos
...
+ Variables explicativas: Las otras 80

```{r}
modelo1 <- rpart(formula = Sale_Price ~ ., data = datos_train, method  = "anova")
# method  = "anova" le está indicado que este es un árbol de regresión
# pues la variable Sale_Price` es numéricas hecha y derecha
```

¿Qué tipo de objeto es modelo1?
...
modelo1 |> summary()
```
+ Veamos la representación gráfica de este árbol
...
+ Moraleja: Podemos ``ir jugando'' con el parámetro de complejidad, que en el fondo nos dice cuántas hojas (o nodos terminales) tiene el árbol.

## Segundo modelo

```{r}
# Quiero toda la complejidad posible cp = 0
...
+ En realidad tantas hojas no son necesarias
...
```{r}
# Vamos a definir un mallado de hiper-parámetro a mano
grid_hiperparametros <- expand.grid(
  minsplit = 5:20,
  maxdepth = 8:15
) # Es un mallado de 16 x 8
...
```{r}
# Encontraremos los mejors hiperparámetros por fuerza bruta
# Evaluando la función en un montón de puntos
modelos <- list()
...
+ `modelos` es una lista que tiene todos los objetos tipo `rpart`

```{r}
modelos |> class()
```
+ Si quiero ver el árbol del modelo 25

```{r}
modelos[[25]] |> rpart.plot()
```

+ Si quiero ver el árbol del modelo 88

```{r}
modelos[[88]] |> rpart.plot()
```

+ Hasta el momento tengo 128 modelos entrenados
...
```{r}
resumen_modelos |> tail()
```

+ Vamos a quedarnos con el del menor error

```{r}
resumen_modelos |> dplyr::arrange(error) |> head()
```

+ Vamos a ordenar los resultados por el menor cp

```{r}
resumen_modelos |> dplyr::arrange(cp) |> head()
```

+ Vamos a quedarnos con el mejor modelo de entre esos 128 posibles
...
```
+ Voy a volverlo a entrenar con estos hiperparámetros ganadores
...
+ Calculemos el ECM de este mejor modelo de árbol
